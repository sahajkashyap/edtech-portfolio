...
> Destined for `.claude/skills/link-verify/SKILL.md` — move it there in a
> session with permission to write inside `.claude/skills/`. Until then, the
> standing rule in `CLAUDE.md` keeps it active.

**Use at the END of any project or change that adds, moves, renames, or
restyles pages or links, and BEFORE telling Sahaj a build is "done and
working." Run it as part of the final review, ideally by a separate reviewing
agent.**

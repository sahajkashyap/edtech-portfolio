...
# The suffixes above can be generated by rule. Derivational affixes cannot:
# "unhappy" is a word and "unbed" is not, and no rule separates them. So they
# are listed by hand -- the same conclusion the er words reached -- and each
# still has to clear the auditor and the age gate before it enters the bank.
#
# Without this, eleven lessons taught an affix that appeared nowhere in their
# own bank, so their warm-up strips drilled unrelated words: Lesson 124 taught
# -ness off a strip reading "bed lamp dark room". The key is the lesson that
# TEACHES the affix -- kindness waits for 124 even though its letters are all
# taught long before.
DERIVED = {
    102: "helpful thankful useful useless harmless careful restful spotless "
         "painless endless cheerful",
    103: "unhappy unwrapped unlock unlocked unpack unpacked unkind unfit "
         "untie untied unzip unsafe unfair",
    104: "preschool reread rereads remake refill refills retell replay "
         "repack reheat repaint",
    105: "disagree disagrees dislike dislikes disliked disappear disappears",
    119: "question questions station stations vacation action actions "
         "motion mention section",
    120: "picture pictures adventure adventures mixture nature future capture",
    121: "painter farmer artist visitor teacher singer actor sailor dentist "
         "florist runner helper",
    124: "kindness darkness happiness sadness softness thickness brightness "
         "stillness",
    125: "basement excitement movement agreement payment statement pavement",
    126: "comfortable breakable enjoyable unbreakable washable readable "
         "sensible possible",
    127: "bicycle bicycles tricycle tricycles unicorn unicorns uniform triangle",
}
...
def build():
    ...
    # hand-listed affixed words, held back to whichever comes later: the lesson
    # that teaches the affix, or the lesson that makes the letters readable
    derived_in, derived_out = 0, []
    for affix_lesson, listing in DERIVED.items():
        for form in listing.split():
            if form in words or form in BLOCKED or form in extra:
                continue
            if not word_age.check(form)[0]:
                derived_out.append(form)
                continue
            rule = earliest_lesson(form)
            if rule is None:
                derived_out.append(form)
                continue
            extra[form] = max(affix_lesson, rule)
            derived_in += 1
    ...
    print(f"wrote {OUT}")
    print(f"  {derived_in} hand-listed affixed words added; "
          f"{len(derived_out)} rejected as too old or not decodable: "
          f"{' '.join(sorted(derived_out)) or 'none'}")
